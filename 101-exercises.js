...
function endsWithVowel(str) {
  var aStr = str.split("");
  var reverseArray = aStr.reverse();
  var bString = reverseArray.join("");
  if (!isVowel(bString[0])) {
    return false;
  } else {
    return true;
  }
...
function firstToLast(sequence) {
  var moveToEnd = sequence.shift();
  sequence[sequence.length] = moveToEnd;
  return sequence;
}
...
function lowestPriceBook(arr) {
  ...
  for(var i=0;i<arr.length;i++) {
    if(arr[i].price < gucciBook.price) {
      gucciBook = arr[i];
    }
  }
  return gucciBook;
...
function numberOfItemTypes(obj) {
  ...
  for(var i=0;i<obj.items.length;i++) {
    if(unique.includes(obj.items[i].title)) {
      continue;
    }
    unique.push(obj.items[i].title);
  }
...
function totalNumberOfItems(obj) {
  var sumOfAllItems = 0;
  for(var i=0;i<obj.items.length;i++) {
    sumOfAllItems += obj.items[i].quantity;
  }
...
function mostSpentOnItem(obj){
  ...
  for(var i=0;i<obj.items.length;i++) {
    if((obj.items[i].price * obj.items[i].quantity) > (example.price * example.quantity)) {
      example = obj.items[i];
    }
  }
  return example;
